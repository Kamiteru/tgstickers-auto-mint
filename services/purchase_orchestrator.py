...
class PurchaseOrchestrator:
    ...
    def calculate_max_purchases(
        ...
        character_price_ton: float
    ) -> Tuple[int, float]:
        """Calculate maximum number of purchases possible with available balance"""
        cost_per_purchase = character_price_ton  # API only supports count=1
    ...
    async def execute_parallel_payment_purchases(
        ...
        # Get character info to calculate real stickers count
    ...
    async def execute_single_method_purchases(
        ...
        results = []
        count = 1  # API only supports count=1
        ...
        try:
            ...
            if active_method == 'STARS':
                ...
                # Limit by available stock (each purchase = 1 pack)
                if character.left < max_purchases:
                    logger.info(f"Limiting purchases to {character.left} due to stock availability")
                    max_purchases = character.left
            ...
            # Calculate real stickers count using character data
            stickers_per_pack = character.stickers_count if character else 1
            total_stickers = successful * stickers_per_pack
    ...
    async def execute_purchase(
        ...
        count = count or 1  # API only supports count=1
        ...
        try:
            ...
            if payment_method == 'STARS':
                ...
                if self.cache:
                    ...
                    wallet_info = await self.wallet.get_wallet_info()
                required = character_price_ton + settings.gas_amount
